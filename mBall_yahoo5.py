...
def main():
    # check_if_Excel_runs()
    # wb = r'C:/Users/[USERNAME_MASKED]\Desktop\00.myself\04.Buyer\0.リサーチ\keyword\key_generator.xlsx'
# データベースの接続情報
#     connection_config = {
#         'user': 'kazuki005', # 'postgres',
#         'password': 'Larc-1225', # [PASSWORD_MASKED]
...
#         'postgresql://postgres:Larc-1225@localhost:5432/scraping'.format(**connection_config))
    ...
    global engine
    engine = create_engine(
        'postgresql://[DB_USER_MASKED]:[DB_PASSWORD_MASKED]@[DB_HOST_MASKED]:5432/[DB_NAME_MASKED]'.
    ...
    for mainKey, categ_num, keyPhrase in list(zip(mainKey_list, categ_num_list, keyPhrase_list))[:3]: #[44:]: #[36:]: # [:2]
        ...
        while i < 61: # 151 20ずつ or 50ずつ繰り上がる
            ...
            elems = soup.select("a[class='Product__titleLink js-rapid-override js-browseHistory-add']")

            url2 = 'https://page.auctions.yahoo.co.jp/jp/auction/'

            for a in elems: # 2021/9/27 ヤフオクのHTMLが変更されていたので
                print(a)
                # elem_url = a.get('href')
                # id
                elem_id = a.get('data-auction-id')
                itemid_list.append(elem_id)
                print(elem_id)
                # url
                elem_url = url2 + '/' + elem_id
                print(elem_url)
                url_list.append(elem_url)
                # title
                title = a.get('data-auction-title')
                print(title)
        ...
        title_df = pd.read_sql('atklist4', con=engine,
        ...
        # merge
        merge = df1.merge(title_df[on], on=on, how='left', indicator=True)
    ...
    # macro()
    # shutil.move('C:/Users/[USERNAME_MASKED]/Desktop/00.Myself/04.Buyer/1.利益計算/db_check_yahoo_elem.txt',
...
#         arr_img = np.r_[arr_img, img_np]
    ...
    相場価格 = concat3['相場価格']
    # print(相場価格)
    # f = lambda x: 現在価格*4 if np.isnan(x) else 現在価格*1.5
